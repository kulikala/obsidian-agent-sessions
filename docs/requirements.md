...
- R-T5a **Shift+Enter・Option+Enter・Cmd+Enter は改行**として Claude Code に届く（プロンプト内で行を増やせる）。
- R-T5b 設定で「Enter の役割」を **送信／改行** から選べる。改行にすると Enter が改行、修飾キー付き Enter が送信になる。実体は Claude Code の `keybindings.json`（`Chat` コンテキスト）で、設定画面は開くたびに現在の値を読んで表示する。Claude Code 全体に効くことを画面で伝える。
- R-T6 複数タブを同時に開いても、各タブが独立して安定する。タブを閉じた後に出力が漏れたり、メモリが残ったりしない。

### R-S セッション管理（GUI）

- R-S0 ビューは 3 種：**サイドパネル**（右サイドバー。一覧・メニュー・ステータスバー）、**セッションマネージャー**（メインエリアのタブ。全セッションの木と管理操作。「新しいタブ」相当の既定ビュー）、**ターミナル**（メインエリアのタブ）。
...
- R-S4 サイドパネルの一覧は「開いているタブ → 起動中（タブなし）→ 最近 N 件」。状態が同期し、前面のタブの行が強調され、クリックでそのタブに切り替わる。
- R-S5 行のクリックでそのセッションのタブを開く。**同じセッションのタブが既にあればそこへ移動する**。同じセッションを 2 つのタブで開くことはない。
- R-S6 行を選択するとその行に `⋯` が出て、`名前を変更`（`/rename`）・`圧縮`（`/compact`）・`アーカイブ`・（起動中なら）`セッションを終了`・`フォルダを開く`・`ID をコピー` を選べる。右クリックも同じ。メニュー行（新規セッション・セッションマネージャー・⋯）にはセッションへの操作を置かない。
...
- R-S9 管理情報（折畳・アーカイブ・未適用の名前変更）は `<vault>/.agents/sessions/sessions.json` に置く。`claude-sessions.md` は廃止。
- R-S10 マネージャーを開いても claude は起動しない。**新規を選ぶか、行をクリックしない限り新しいセッションは始まらない。**
- R-S11 行を指すと直近の指示・直近の応答・使ったツール・フォルダ・ID が詳細欄に出る（サイドパネル・マネージャー共通）。
...
- R-A3 サイドパネル下部のステータスバーに、前面のターミナルタブのセッションについて、現在のモデル・エフォート・コンテキスト使用率・Remote Control の状況・5 時間／7 日の利用率を出す。取れない項目は「—」。
