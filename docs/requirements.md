...
- R-S7 新規セッションはダイアログから。「カテゴリ」（既存のカテゴリを候補に出す）と「名前」の 2 欄で、名前は任意（空でも開始できる）。名前変更のダイアログも同じ 2 欄で、いまの名前を分けて入れる。作業フォルダは vault 直下。開始と同時に名前があれば `/rename` を送る。
...
- R-S11 詳細欄（サイドパネル・マネージャー共通）には、名前、モデル・エフォート・rc のバッジ（rc は未接続・不明が ○、接続中は緑の ●）、コンテキスト使用率の円グラフ、総トークン・総コスト、直近の指示・応答（行数を絞り、クリックで全文）、ツール・フォルダ・ID を出す。何も指していなければ前面のタブのセッションを出す。
- R-S12 マネージャーは **利用状況の分析** の画面：上部に 5 時間枠・7 日枠の統計（使用率、リセットまでの残り、枠内のコスト・トークン・呼出数・セッション数をラベル付きで）とカテゴリ別（7 日枠）のコストの帯、グループ見出し行にカテゴリの 5h／7d の合計、表（印・名前・最終更新・5h のコスト・7d のコスト・フォルダ、グループ見出しに件数、行は 32px 以上、列は重ならない）、見出しで並べ替え（最終更新＝木、5h／7d＝コストの降順）、右の詳細パネル。↑↓／Enter／`/` で操作できる。ツールバーはアイコン（＋・再走査）と絞込、⋯ に「アーカイブを表示」のチェック。サイドパネルは **今の作業** の画面。
...
- R-A1 タブのアイコンは状態ごとに違う（接続中・処理中・コマンド実行中・指示待ち・編集中・待機・未接続・終了・エラー）。色と動き（回転・点滅・脈動）で見分けられ、`prefers-reduced-motion` では動かない。サイドパネル・マネージャーの行の印も同じ状態・色。マネージャー（`layout-dashboard`）・サイドパネル（`list-tree`）のアイコンはターミナルと違う。
