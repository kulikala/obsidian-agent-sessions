...
- R-M1 Settings has an "Agents" section: a sub-heading for each of Claude Code and Codex (icon + name, its enabled toggle on the same row), then that agent's own path (empty = auto-detect), multi-line environment variables (`KEY=VALUE` per line), and detection result — rather than one flat list where both agents' identically-named fields were told apart only by position (T-117). At least one agent stays enabled at all times. A disabled agent's path/environment/detection rows are dimmed, not hidden.
- R-M2 The first time the plugin ever runs with no saved agent settings, it auto-detects each agent — a login shell's `command -v`, then the same probe in an interactive shell (catches a version manager like mise/asdf/nvm/volta whose shell integration only loads interactively), then a list of common install locations aware of those version managers — and enables whichever it finds; if neither is found, Claude Code stays enabled (unchanged from before multi-agent support). Each agent has its own "Find again" button (T-117) that re-detects just that one agent any time and shows the result and detected version, without changing any toggle itself (unless it found a still-disabled agent, which shows an inline "Enable" button next to the result).
...
- R-M5 The submit-key setting and Enter interception (R-T5a–d) apply to both Claude Code and Codex tabs (R-M10); every other agent's own keymap is left untouched.
...
- R-M10 The submit-key setting also applies to Codex, by writing its own `config.toml` (under `CODEX_HOME`, default `~/.codex`) — one fixed key pair whenever the setting isn't Enter (`tui.keymap.composer.submit`/`tui.keymap.editor.insert_newline`, since Codex's own keymap has no Cmd/Super modifier to bind a per-choice key to; the plugin's own key interception translates whichever of the 4 non-Enter choices is configured into the same bytes either way), removed again when it's set back to Enter. Every line this writes is marked so `uninstall.sh`/`setup --remove` can clean it up without disturbing anything else in the file; a pre-existing, unmarked key for the same purpose is never overwritten (a warning names it instead). Independently, if Codex's own `status_line` isn't set at all, the plugin writes a default one that mirrors Claude Code's own statusLine content — never overwriting an already-set one, the plugin's own past write included. Codex's own terminal-title text (R-A1's title-based signal) is read but never written to.
...
- R-S4 The side panel's list is "open tabs → running (no tab) → recent, N items". Each row shows a category chip, then the name with the category stripped, then a relative last-updated time ("just now" / "N min ago" / "N h ago" / "yesterday" / "N d ago", falling back to a locale-formatted short date past a week, e.g. "9/25" or "2025/09/18" with the year included once it's not the current one, T-115; the absolute date and time is in a tooltip). Status stays in sync, the frontmost tab's row is highlighted (bold, a background, and a left accent bar — still distinguishable from an asking/waiting row's own, more urgent styling when both apply at once), and clicking a row switches to that tab. The name never shrinks past a minimum width — if the row gets too narrow to fit everything, the time is what disappears, not the name. The rate-limits area shows usage percentages and a countdown to reset (no "resets in" label, just the time itself) for every enabled agent (T-104 — a small agent icon at each row's head), laid out so all the bars are always the same length regardless of how wide the percentage/countdown text is. Each row's own window label is always the compact form ("5h"/"7d"/"30d", T-113 — the long form, e.g. "5-hour window", is in a tooltip instead; too narrow an area for the long form to ever be worth showing directly, unlike the manager's own wider stat cards). Claude always shows its 5-hour/7-day pair; another agent shows one row per window it actually reports a tracked percentage for (T-104 addendum) — the usual 5h/7d pair, a single non-standard window (e.g. a 30-day-only plan), more, or none. Clicking the area re-reads/re-fetches the latest usage data immediately, for every shown agent.
- R-S4a With no sessions at all in any of the three sections, the list shows a centered wordmark, a one-line description, and a "New session" button in place of being blank. If no agent is enabled, or none of the enabled agents' binaries can be found, that button is replaced with one that opens settings instead. The rate-limits area is unaffected either way — it's shown whenever there's usage data on disk to show, regardless of the session list.
...
- R-S6 Every row's right edge always has a `⋯` (rename via `/rename`; move to category, disabled when the session has neither a name nor a first prompt yet to attach one to; compress via `/compact`, disabled if the last command already was `/compact`; archive, or unarchive if already archived; end session, when running; session analysis; copy ID). Right-click opens the same menu. No session-level action lives in the nav row (new/manager/`⋯`).
...
- R-S12 The manager is the **usage-analysis** view: a session list on top, a resizable analysis area below (height persists; the whole area's own resize and click-to-refresh apply regardless of how many agents are shown). With 0 or 1 agent enabled, analysis is one panel under a single "Analysis" heading that folds the whole area (state persists) — unchanged from before multi-agent support. With more than one agent enabled, that single wrapper is gone: each agent gets its own independent, top-level panel with its own heading ("Claude Code analysis"/"Codex analysis", icon included), always stacked one on top of the other regardless of width (T-114); each panel folds on its own (persisted per agent), independent of the others, and a folded panel still shows a small usage percentage next to its heading. Claude's panel always shows its 5-hour/7-day pair, even as placeholders before the first fetch; every other agent's panel only shows a stat card and a per-category cost bar for a window it actually has tracked usage for (T-104 addendum, T-113 — not a fixed 5-hour/7-day pair, and no placeholder for a window it was never tracking at all; a window's own length picks its label, e.g. "5-hour window"/"7-day window"/"30-day window"; a one-line message instead of any cards when that agent has no tracked window yet), counting only that agent's own sessions. Cost and call/session counts of 1,000 or more are comma-grouped throughout the analysis area (and everywhere else a raw, non-abbreviated count or a dollar amount is shown — tables, cards, bars, the detail pane, the session-analysis modal, tooltips), in the plugin's own display language. The list itself stays one combined, agent-mixed table regardless (not split by agent) — a sortable table (state mark, name, last-updated, model, effort, 5h cost, 7d cost, folder — full model/effort values in a tooltip; group headings show a category chip, count, and 5h/7d totals; rows are at least 32px tall, with columns wide enough that their own header text and typical values never clip, and that never overlap; a category name never appears twice in one row — inside a group, rows show only the name), with each row's 5h/7d cost attributed to its own agent. Sorting by last-updated shows the grouped tree; sorting by 5h/7d cost flattens it, descending (also per-row-agent-aware). Keyboard: ↑↓/Enter/`/`. The toolbar has icon buttons (new, rescan), a name filter, a status-filter menu (all / needs input / needs review / running / done / archived, matching Claude's own app's filter — the selection persists), and a "Show archive" toggle in `⋯` (governs whether archived sessions are included when the status filter is "all"). The side panel is the **current-work** view.
...
- R-S14 "Move to category…" (from the row menu) is a single category-only field: focusing or clicking it shows a dropdown of existing categories (with their color chip and count), narrowed as you type (a prefix match ranked ahead of a substring-only match); freehand text for a new category is also accepted; confirming empty removes the category, leaving just the session's label. The current (or just-confirmed) category shows as a colored chip in the field, removable via a small "×" on it or Backspace, the same way the naming dialog's category chip works; the suggestion dropdown is never clipped by the dialog's own height, however many categories there are. Confirming applies through the same path as a rename (R-S8). The session's own label (its name, minus any category, or — if it hasn't been named yet — its auto-derived label from the first prompt) is never shown for editing here; "Rename" is for that.
- R-S15 Wherever a session's name is shown (tab title, side panel, manager, detail pane, notifications, row menu, an archived entry's cached name) — a single rule, applied everywhere: the session's own name if it has one, otherwise its auto-derived label if that's real content, otherwise "New Claude Code session"/"New Codex session" (by agent). The session's own id is never shown as its name anywhere, including for a deferred tab, a not-yet-resolved Codex session, or right after a restart before the first scan completes — none of these ever show a placeholder built from the id.
