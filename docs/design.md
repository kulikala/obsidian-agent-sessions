...
Output (T-103): `{"windows":{"five_hour":W,"seven_day":W},"agents":{"claude":{"windows":{...}},"codex":{"windows":{...}}}}`, where `W = {"start","end","used_percentage","total":{calls,input,output,cache_read,cache_create,cost},"sessions":{id:{calls,input,output,cache_read,cache_create,cost}}}` — the same shape for every agent, so the plugin's analysis views (§10.2's stat cards/category bars, §8's rate-limit bars) can render any of them through the same code. `end` is `resets_at` (or the current time if absent); `start` is `end − 5h`/`7d`. `used_percentage` comes from `rate_limits` (`null` if unavailable). Only an *enabled* agent (§3.3's `enabled_agents()`) gets an `agents.<name>` entry.
