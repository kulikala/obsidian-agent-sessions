...
1. **A tab is open and attached**: write to the PTY from that tab. Looked up by the *daemon-tracked* id (`main.ts`'s `daemonIdFor`), which can differ from the id the caller (row menu, dialog) knows for a linked Codex session — §7.7.
2. **No tab, but the daemon has it running**: attach temporarily (120×40) under the daemon-tracked id, write, detach.
...
**Auto-detection** (`backend.ts`'s `detectAgents`): a login-shell `command -v <bin>` first, then a fixed list of common install locations (`~/.local/bin`, `/opt/homebrew/bin` on macOS, `/usr/local/bin`, `~/.npm-global/bin`, and npm's own `npm config get prefix`/bin). Runs automatically exactly once — the very first time saved settings have no `agents` object at all (`main.ts`'s `needsAgentDetection`, checked against the *raw* loaded data before `mergeSettings` fills in defaults) — and enables whichever it finds; neither found leaves Claude enabled (the pre-T-96 default, unchanged). The settings tab's "Detect again" button re-runs it any time and shows each agent's result inline, but — unlike the first-run case — never flips a toggle itself; the user decides what to do with the result.
...
**New-Codex-session id resolution** (`main.ts`'s `resolveCodexSession`, `backend.ts`'s `resolve`): since Codex's real id isn't known until it exists, `newSession` for Codex opens the tab under a plugin-generated placeholder id (daemon-only — the tab talks to the daemon under this id for its entire life; nothing renames a running daemon session) and kicks off `resolveCodexSession(placeholderId, cwd)` in the background: finds the placeholder daemon session's own pid (polling `client.list()`, since `start`'s own promise doesn't guarantee the daemon call has completed), then polls `agent-sessions json resolve codex --pid PID --since EPOCH --cwd CWD` (§3.3 documents the two strategies Python tries) until it returns a real thread id — `{thread: null, transcript: null}`, not an error, means "not yet" and the caller retries; both stages give up silently after a bounded number of attempts (the tab keeps running regardless; it just won't show up in the row list, or route correctly from a row-menu action, until linked). Once resolved, `sessions.json`'s `sessions[thread] = {agent: "codex", cwd, daemon: placeholderId}` is written (`types.ts`'s `StoreSessionEntry.daemon`) and a rescan follows — Python's `json live`/`json scan` do the id-relabeling from there (§3.3's `_daemon_list`), so the row appears under `thread` like any other session. A brand-new Codex session isn't named at creation (`notice.renameAtCreateUnsupported` — §6's `waitFor`/`waitForName` only work through the registry/scan, which for a not-yet-linked session are watching an id nothing is filed under yet); rename it once it's running instead. `main.ts`'s `daemonIdFor(id)` is the reverse lookup `sendCommand` (§6) uses — `sessions.json`'s `daemon` field on `id`'s own entry, falling back to `id` unchanged for everything else (Claude, or a Codex session not yet linked).
