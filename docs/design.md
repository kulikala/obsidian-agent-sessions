...
- **Scan**: enumerates `$CODEX_HOME/sessions/**/rollout-*.jsonl` (`CODEX_HOME` env, default `~/.codex`); reuses `sessions.scan`'s `RACY_WINDOW`/cache-entry shape (both agents share one `scan-cache.json`, keyed by path, so no collision). `child` is `session_meta.payload.source != 'cli'` (a launch from vscode/exec/another tool, mirroring Claude's own child-session rule). Name resolution: `state_5.sqlite`'s `threads.name` (an explicit `/rename`, read via a `mode=ro` sqlite URI — never a write, never a lock) if present, else the first user message, same convention as an untitled Claude Code session. That first message comes from `event_msg.user_message` specifically, never `response_item`'s role=user (which is Codex's own reconstructed prompt for the model and mixes in injected context — AGENTS.md instructions, `<environment_context>`, etc; real local data showed these leaking into session names before this was fixed), and a bare slash command (`/exit`, `/compact`, ...) is skipped too, same as an injection marker (`agents/codex/rollout.py`'s `INJECTED_PREFIXES`/`is_real_user_text`). Unlike Claude, a session where nothing survives this filtering is still shown, just nameless (in "Other") — the naive first line being unusable doesn't mean the session itself is empty.
