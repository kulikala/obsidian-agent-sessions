...
**Not applied to the terminal tab's own icon** (a judgment call, T-102): the tab header has exactly one icon slot, already showing the session's live status (§7.5's `TERMINAL_STATUS_ICON`, driven by `updateIcon()`/`headerIconEl()`) — replacing it with a static agent mark would drop that status information, and there's no supported way to show a second icon in Obsidian's own tab header without poking unsupported internal DOM beyond what `headerIconEl()` already does. Flagged rather than guessed.
