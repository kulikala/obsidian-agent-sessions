...
| `agent-sessions json stats` | 5-hour/7-day window aggregation across all Claude Code sessions (§13.2); additionally `{"agents":{"codex":{"windows":{"primary","secondary"}}}}` when Codex is enabled (§3.3) — absent entirely otherwise, so this key never appears for a deployment that predates it |
