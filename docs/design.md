...
**New-session dialog**: `NewSessionModal` shows an agent dropdown only when more than one agent is enabled (with exactly one, that one is used without asking); it defaults to `settings.lastNewSessionAgent`, which is overwritten with whatever was actually chosen. `main.ts`'s `newSession(name?, agent = settings.lastNewSessionAgent)` threads the choice through to `sessions.json` and `openSession`.
...
**Display**: an agent mark (§8, §10.1 — a small lucide icon, deliberately not a brand logo; `rows.ts`'s `AGENT_ICON`/`AGENT_NAME_KEY`/`renderAgentMark`) sits next to the state mark on every row, and the detail pane's badge row (§9) leads with the same name. Sessions from every enabled agent are listed, sorted, grouped, and filtered together, with no agent-specific split — the manager's status-filter menu (§10.1) is deliberately agent-blind for the same reason.
