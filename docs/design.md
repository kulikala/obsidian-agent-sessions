...
| `$CODEX_HOME/state_5.sqlite` | Codex's own thread index, including `/rename`d names (`threads.name`) | Codex CLI (read-only from this project, opened `mode=ro`, never locked or written) |
...
**Model/effort with no statusLine**: Codex has no statusLine (§14 is Claude-only), so the detail pane's model/effort badges (§9) fall back to `json detail`'s own `model`/`effort` fields (additive — present only for an adapter that populates them; absent, not `null`, otherwise) once `statusInfo` (Claude's live statusLine data) has nothing.
...
4. **Rate limits** (`views/limits.ts`): one 5h/7d pair of rows per enabled agent (T-104), each with a small agent icon at its head (`AGENT_ICON_ID`), a usage-percentage bar, and the countdown to reset — `h:mm:ss`, or "N days h:mm" past 24 hours, with no "resets in" label — refreshed every second. `resetsAt` missing, or `usedPercentage` missing, shows "—" (the latter is a real, possibly-permanent state for a Codex account on a plan with no 5h/7d tracking at all, not just "no data yet"). Each row is a fixed-column grid (agent icon, label, bar, %, countdown) so the bar's rendered width never depends on how wide its neighbors' text is. **Two different sources, both converted to the same internal `RateLimitWindow` shape** (`{usedPercentage, resetsAt}`) so both render through the identical `renderWindow`: Claude's rows read the most recently modified `status/*.json` file that carries `rate_limits` (account-wide, written in real time by its own statusLine hook; this assumes one account isn't used concurrently from two places) — unchanged from before T-104. Every other enabled agent's rows (Codex) instead poll `agent-sessions json stats`'s `agents.<agent>.windows` (§10.2's same per-agent shape) every 60 seconds, converted via `fromStatsWindow` (`w.end` stands in for `resetsAt` — Codex's own more precise `rate_limits.resets_in_seconds` isn't part of this shape, a judgment call flagged for review, not confirmed with lnx-py). Either way, if the window's reset time has passed but a fresh value hasn't arrived yet, `rollForwardWindow` (the same rule as §10.2's pure function) advances it so "the current window" is what's shown; since the redraw happens every second and re-evaluates `Date.now()` each time, the display flips over naturally the instant the reset time passes. Clicking the area re-reads/re-fetches every shown agent's source immediately rather than waiting for the next automatic refresh (rapid clicks collapse into one, with a brief dimmed/`cursor: progress` state) — for Claude this doesn't make fresher `rate_limits` values appear, since those are only ever as current as the last time its own statusLine hook wrote them (it just re-reads whatever is already on disk), but for a `json stats`-sourced agent it's a real re-fetch. The shown agent set (`refreshAgents`, called from `SideView`'s existing `settings-changed` handler) is rebuilt whenever the enabled-agent set changes, without needing a full side-panel reload.
...
**Empty state** (T-104): when the list has nothing in any of the three sections, `renderEmptyState` shows a centered wordmark ("Agent Sessions", not localized — a product name), a one-line description (`empty.desc`), and a "New session" button (`mod-cta`, the same action as the nav's `+`) in place of the (otherwise blank) list. If no agent is enabled, the button is a "Open settings" one instead (`empty.noAgentEnabled`) — there's nothing to launch. If at least one is enabled, the "New session" button shows first and a background check (`resolveAgentBinary` against every enabled agent, via `Promise.allSettled`) swaps it for the same settings fallback (`empty.noAgentFound`) if none of them can actually be found — never awaited inline, so it never delays the rest of `render()`. The rate-limit bars (item 4 above) are a separate, independent DOM area (`limitsHostEl`, built once in `buildSkeleton`) and are shown regardless of whether the list is empty, as long as there's `rate_limits` data on disk to show.
...
- Badges: agent (§7.7's display name — "Claude Code"/"Codex"), model, effort (from `statusInfo` — Claude's own live statusLine data — falling back to `json detail`'s `model`/`effort` for an agent with no statusLine, "Default" if neither has a value), and rc (a green ● when connected, ○ otherwise or unknown — the same look as §10.1's rc chip).
...
The dropdown itself (`modals.ts`'s `buildCategorySuggest`) is a small shared component — chip+label rows, arrow-key/mouse highlighting, click-or-Enter confirmation via a caller-supplied callback — used by both this composed field and the "Move to category" dialog below, which differ only in what confirming a suggestion does with it.
...
**Move to category** (T-102, the row `⋯` menu's "Move to category…"; `rows.ts`'s `RowActions.moveToCategory`, `modals.ts`'s `MoveToCategoryModal`): a single category-only field (the same `buildCategorySuggest` dropdown, opened on focus or click as well as while typing, since there's no chip-vs-typing state to gate it on here) — the session's own label is never shown for editing in this dialog. Confirming composes `Category: Label` (or just `Label` with an empty category, removing it) and sends it through the exact same path as a rename (`plugin.renameSession`, `/rename` either way, regardless of agent). The label used is `categorizableLabel(row)` (`name.ts`): the name's own label if the session has already been named, otherwise the row's auto-derived label (its first prompt, truncated) — never the "Untitled `<id>`" placeholder shown elsewhere in the UI, which isn't real content to categorize. The menu item is disabled when `categorizableLabel` is empty (a session with neither a name nor a first prompt yet) — a judgment call: inventing a label (a literal "無題"/"Untitled" string) just to attach a category to it would write meaningless content into the session's real name, so the action is unavailable until there's something real to categorize; "Rename" (which sets both at once) remains available regardless.
